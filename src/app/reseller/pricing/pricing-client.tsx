...
const EXAMPLE_PACKAGES = [
  { name: "1GB Data Bundle", cost: 5.00 },
  { name: "2GB Data Bundle", cost: 10.00 },
  { name: "5GB Data Bundle", cost: 20.00 },
];
...
export default function PricingClient({ userId, currentMarkup }: PricingClientProps) {
  ...
  const handleSaveMarkup = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to save markup",
        variant: "destructive",
      });
    } finally {
      setSaving(false);
    ...
    <div className="container mx-auto max-w-4xl px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Set your profit margin. This percentage markup will apply to all packages.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Set your markup percentage (5% - 20%). This will be applied to all package prices.
            Example: If a package costs GHS 10 and you set 10% markup, customers will pay GHS 11.
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <div className="border rounded-lg p-4 bg-muted/50">
              ...
              <div className="space-y-2">
                {EXAMPLE_PACKAGES.map((pkg) => (
                  <div key={pkg.name} className="flex justify-between items-center text-sm">
                    <span className="text-muted-foreground">{pkg.name}</span>
                    <div className="text-right">
                      <span className="text-muted-foreground">Cost: GHS {pkg.cost.toFixed(2)} → </span>
                      <span className="font-semibold">
                        GHS {calculateSellingPrice(pkg.cost).toFixed(2)}
                      </span>
                    </div>
                  </div>
                ))}
