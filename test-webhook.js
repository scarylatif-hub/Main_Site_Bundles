...
async function testWebhook() {
  console.log('Testing Dakazina webhook integration...');
...
async function testMultipleStatuses() {
  const statuses = ['PROCESSING', 'DELIVERED', 'CANCELED'];
  ...
  for (const status of statuses) {
    ...
    const payload = {
      ...
      reference: `REF-TEST-${Math.random().toString(36).substring(7).toUpperCase()}`,
      order_code: `DKZ-TEST-${Math.random().toString(36).substring(7).toUpperCase()}`,
    };
    ...
    try {
      ...
      const responseText = await response.text();
      console.log(`Status ${status}:`, response.status, response.ok ? '✅' : '❌');
...
if (require.main === module) {
  testWebhook().then(() => {
    console.log('\n--- Testing multiple statuses ---');
    return testMultipleStatuses();
  });
}

module.exports = { testPayload, testWebhook, testMultipleStatuses };
